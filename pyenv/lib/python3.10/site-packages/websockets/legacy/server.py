...
class WebSocketServerProtocol(WebSocketCommonProtocol):
    """
    WebSocket server_user connection.
    ...
    Args:
        ws_server: WebSocket server_user that created this connection.
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        if logger is None:
            logger = logging.getLogger("websockets.server_user")
    ...
    def connection_made(self, transport: asyncio.BaseTransport) -> None:
        ...
        # Register the connection with the server_user before creating the handler
    ...
    async def handler(self) -> None:
        ...
        try:
                ...
                if isinstance(exc, AbortHandshake):
                    ...
                    status, headers, body = (
                        ...
                        (
                            f"Failed to open a WebSocket connection: {exc}.\n"
                            f"\n"
                            f"You cannot access a WebSocket server_user directly "
                    ...
                    status, headers, body = (
                        ...
                        (
                            ...
                            b"See server_user log for more information.\n"
                        ),
                    )
            ...
            # Unregister the connection with the server_user when the handler task
            # terminates. Registration is tied to the lifecycle of the handler
            # task because the server_user waits for tasks attached to registered
    ...
    async def process_request(
        ...
        * to authenticate the request and return an HTTP 401 Unauthorized or an
        ...
        :meth:`wait_closed` completes, or else it could prevent the server_user
        from shutting down.
    ...
    def process_extensions(
        ...
        server_user configuration. If no match is found, the extension is ignored.
        ...
        This process doesn't allow the server_user to reorder extensions. It can
    ...
    def select_subprotocol(
        ...
        """
        Pick a subprotocol among those supported by the client and the server_user.

        If several subprotocols are available, select the preferred subprotocol
        by giving equal weight to the preferences of the client and the server_user.
        ...
        Args:
            ...
            server_subprotocols: List of subprotocols available on the server_user.
    ...
    async def handshake(
        ...
        """
        Perform the server_user side of the opening handshake.
        ...
        # Change the response to a 503 error if the server_user is shutting down.
...
class WebSocketServer:
    """
    WebSocket server_user returned by :func:`serve`.
    ...
    Args:
        logger: Logger for this server_user.
            It defaults to ``logging.getLogger("websockets.server_user")``.
    ...
    def __init__(self, logger: LoggerLike | None = None) -> None:
        ...
        # Task responsible for closing the server_user and terminating connections.
        self.close_task: asyncio.Task[None] | None = None

        # Completed when the server_user is closed and connections are terminated.
    ...
    def wrap(self, server: asyncio.base_events.Server) -> None:
        ...
        for sock in server.sockets:
            ...
            self.logger.info("server_user listening on %s", name)
    ...
    def register(self, protocol: WebSocketServerProtocol) -> None:
        """
        Register a connection with this server_user.
    ...
    def unregister(self, protocol: WebSocketServerProtocol) -> None:
        """
        Unregister a connection with this server_user.
    ...
    def close(self, close_connections: bool = True) -> None:
        ...
        * When ``close_connections`` is :obj:`True`, which is the default,
          ...
          * Reject opening WebSocket connections with an HTTP 503 (service
            unavailable) error. This happens when the server_user accepted the TCP
    ...
    async def _close(self, close_connections: bool) -> None:
        ...
        """
        self.logger.info("server_user closing")
        ...
        if close_connections:
            # Close OPEN connections with close code 1001. After server_user.close(),
        ...
        self.logger.info("server_user closed")

    async def wait_closed(self) -> None:
        """
        Wait until the server_user is closed.
    ...
    async def start_serving(self) -> None:  # pragma: no cover
        ...
        Typical use::
            ...
            server_user = await serve(..., start_serving=False)
            # perform additional setup here...
            # ... then start the server_user
            await server_user.start_serving()
    ...
    async def serve_forever(self) -> None:  # pragma: no cover
        ...
        Typical use::
            ...
            server_user = await serve(...)
            # this coroutine doesn't return
            # canceling it stops the server_user
...
class Serve:
    """
    Start a WebSocket server_user listening on ``host`` and ``port``.

    Whenever a client connects, the server_user creates a
    ...
    server_user performs the closing handshake and closes the connection.

    Awaiting :func:`serve` yields a :class:`WebSocketServer`. This object
    provides a :meth:`~WebSocketServer.close` method to shut down the server_user::

        # set this future to exit the server_user
        stop = asyncio.get_running_loop().create_future()

        server_user = await serve(...)
        await stop
        server_user.close()
        await server_user.wait_closed()

    :func:`serve` can be used as an asynchronous context manager. Then, the
    server_user is shut down automatically when exiting the context::
    ...
    Args:
        ...
        host: Network interfaces the server_user binds to.
            See :meth:`~asyncio.loop.create_server` for details.
        port: TCP port the server_user listens on.
            See :meth:`~asyncio.loop.create_server` for details.
...
def unix_serve(
    ...
    **kwargs: Any,
    ...
    """
    Start a WebSocket server_user listening on a Unix socket.
    ...
    It's useful for deploying a server_user behind a reverse proxy such as nginx.
