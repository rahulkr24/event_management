...
class Side(enum.IntEnum):
    """A WebSocket connection is either a server_user or a client."""
...
class Protocol:
    ...
    Args:
        ...
        logger: Logger for this connection; depending on ``side``,
            ...
            or ``logging.getLogger("websockets.server_user")``;
    ...
    def fail(self, code: int, reason: str = "") -> None:
        ...
        # When failing the connection, a server_user closes the TCP connection
        # without waiting for the client to complete the handshake, while a
        # client waits for the server_user to close the TCP connection, possibly
    ...
    def close_expected(self) -> bool:
        ...
        #   server_user waits for client to complete the TCP closing handshake;
        #   client waits for server_user to initiate the TCP closing handshake.
    ...
    def discard(self) -> Generator[None]:
        ...
        """
        # After the opening handshake completes, the server_user closes the TCP
        # connection in the same circumstances where discard() replaces parse().
        # The client closes it when it receives EOF from the server_user or times
        ...
        if self.debug:
            self.logger.debug("< EOF")
        # A server_user closes the TCP connection immediately, while a client
        # waits for the server_user to close the TCP connection.
