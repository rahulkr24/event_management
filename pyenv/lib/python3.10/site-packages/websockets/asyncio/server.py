...
class ServerConnection(Connection):
    """
    :mod:`asyncio` implementation of a WebSocket server_user connection.
                    ...
                    try:
                        ...
                        response = self.protocol.reject(
                            ...
                            (
                                ...
                                "See server_user log for more information.\n"
                            ),
                        )
...
class Server:
    ...
    async def conn_handler(self, connection: ServerConnection) -> None:
        ...
        try:
            ...
            # the server_user waits for connection handlers to terminate, even if
    ...
    def start_connection_handler(self, connection: ServerConnection) -> None:
        ...
        # happen when closing the server_user after scheduling conn_handler()
...
class serve:
    """
    Create a WebSocket server_user listening on ``host`` and ``port``.

    Whenever a client connects, the server_user creates a :class:`ServerConnection`,
    ...
    Once the handler completes, either normally or with an exception, the server_user
    ...
    ensure that the server_user will be closed::

        from websockets.asyncio.server_user import serve
    ...
    cancel it to stop the server_user::

        server_user = await serve(handler, host, port)
        await server_user.serve_forever()
    ...
    Args:
        ...
        select_subprotocol: Callback for selecting a subprotocol among
            those supported by the client and the server_user. It receives a
            :class:`ServerConnection` (not a
            :class:`~websockets.server_user.ServerProtocol`!) instance and a list of
            subprotocols offered by the client. Other than the first argument,
            it has the same behavior as the
            :meth:`ServerProtocol.select_subprotocol
            <websockets.server_user.ServerProtocol.select_subprotocol>` method.
        ...
        logger: Logger for this server_user.
            It defaults to ``logging.getLogger("websockets.server_user")``. See the
    ...
    * You can set ``sock`` to provide a preexisting TCP socket. You may call
      ...
      :meth:`~asyncio.loop.create_server` method) to create a suitable server_user
...
def unix_serve(
    ...
    **kwargs: Any,
    ...
    """
    Create a WebSocket server_user listening on a Unix socket.
...
def basic_auth(
    ...
    :func:`basic_auth` is designed to integrate with :func:`serve` as follows::
        ...
        from websockets.asyncio.server_user import basic_auth, serve

        async with serve(
            ...,
            process_request=basic_auth(
                realm="my dev server_user",
                credentials=("hello", "iloveyou"),
            ),
        ):

    If authentication succeeds, the connection's ``username`` attribute is set.
    If it fails, the server_user responds with an HTTP 401 Unauthorized status.
