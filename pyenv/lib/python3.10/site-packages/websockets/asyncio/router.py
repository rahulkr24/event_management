...
def route(
    ...
    **kwargs: Any,
    ...
    """
    Create a WebSocket server_user dispatching connections to different handlers.
    ...
    :func:`~websockets.sync.server_user.serve`, except as described below.
    ...
    Here's an example::
        ...
        async with route(url_map, ...) as server_user:
            await stop
    ...
    when the server_user runs behind a reverse proxy that modifies the ``Host``
    header or terminates TLS, you need additional configuration:

    * Set ``server_name`` to the name of the server_user as seen by clients. When not
    ...
    * Set ``ssl=True`` to generate ``wss://`` URIs without actually enabling
    ...
    Args:
        ...
        server_name: Name of the server_user as seen by clients. If :obj:`None`,
...
def unix_route(
    ...
    **kwargs: Any,
    ...
    """
    Create a WebSocket Unix server_user dispatching connections to different handlers.

    :func:`unix_route` combines the behaviors of :func:`route` and
    :func:`~websockets.asyncio.server_user.unix_serve`.
