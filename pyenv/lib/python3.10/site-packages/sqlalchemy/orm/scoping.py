...
class scoped_session(Generic[_S]):
    ...
    def bulk_insert_mappings(
        ...
        :param render_nulls: When True, a value of ``None`` will result
         ...
         that server_user-side defaults which are invoked by an omitted column will
         be skipped, so care must be taken to ensure that these are not
         necessary.

         .. warning::

            When this flag is set, **server_user side default SQL values will
            not be invoked** for those columns that are inserted as NULL;
            the NULL value will be sent explicitly.   Care must be taken
            to ensure that no server_user-side default functions need to be
