...
temporary tables as well as other server_user state such as prepared statement
...
previous - if a server_user version below 2008 is detected, DDL
...
of the server_user version in use, if not otherwise set by the user.  The
...
server_user. ``server_version_info`` will always return the database
server_user version information (in this case SQL2005) and not the
compatibility level information. Because of this, if running under
a backwards compatibility mode SQLAlchemy may attempt to use T-SQL
statements that are unable to be parsed by the database server_user.
...
server_user side defaults.   MS-SQL does not
...
class REAL(sqltypes.REAL):
    ...
    def __init__(self, **kw):
        # REAL is a synonym for FLOAT(24) on SQL server_user.
...
class DOUBLE_PRECISION(sqltypes.DOUBLE_PRECISION):
    ...
    def __init__(self, **kw):
        # DOUBLE PRECISION is a synonym for FLOAT(53) on SQL server_user.
...
class MSSQLCompiler(compiler.SQLCompiler):
    ...
    def returning_clause(
        ...
        # SQL server_user returning clause requires that the columns refer to
...
class MSDDLCompiler(compiler.DDLCompiler):
    ...
    def visit_computed_column(self, generated, **kw):
        ...
        # explicitly check for True|False since None means server_user default
...
class MSDialect(default.DefaultDialect):
    ...
    # may be changed at server_user inspection time for older SQL server_user versions
    ...
    # "The incoming request has too many parameters. The server_user supports a "
    ...
    def _setup_version_attributes(self):
        if self.server_version_info[0] not in list(range(8, 17)):
            util.warn(
                "Unrecognized server_user version info '%s'.  Some SQL Server "
