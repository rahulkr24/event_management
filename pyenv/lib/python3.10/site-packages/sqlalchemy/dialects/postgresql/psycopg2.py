...
* ``stream_results`` - Enable or disable usage of psycopg2 server_user side
...
* A common way to set up client encoding with PostgreSQL databases is to
  ensure it is configured within the server_user-side postgresql.conf file;
  this is the recommended way to set encoding for a server_user that is
...
class PGDialect_psycopg2(_PGDialect_common_psycopg):
    ...
    def _is_disconnect_messages(self):
        return (
            ...
            "connection not open",
            "could not receive data from server_user",
            "could not send data to server_user",
