...
server_user version detection scheme that determines if the
...
The above engine, upon first connect, will raise an error if the server_user version
...
accommodation of  server_user restarts and network issues, a pre-ping approach may
...
engine for the table.  Historically, MySQL server_user installations would default
...
server_user side cursors; generally, only SQL statements that return rows should be
...
the server_user if plain ``utf8`` is specified within any server_user-side directives,
...
``utf8``, changes to the MySQL/MariaDB schema and/or server_user configuration may be
required.
...
usual need to use a trigger in such a case where server_user-side update changes are
desired.
...
MySQL 5.6.  With this server_user setting enabled, TIMESTAMP columns behave like
...
class MySQLCompiler(compiler.SQLCompiler):
    ...
    def limit_clause(self, select, **kw):
        ...
        #   LIMIT <limit>
        #   LIMIT <offset>, <limit>
        # and in server_user versions > 3.3:
        #   LIMIT <limit> OFFSET <offset>
        # The latter is more readable for offsets but we're stuck with the
        # former until we can refine dialects by server_user revision.
...
class MySQLDialect(default.DefaultDialect):
    ...
    def _get_server_version_info(self, connection):
        # get database server_user version info explicitly over the wire
    ...
    def _parse_server_version(self, val):
        ...
        if server_version_info < (5, 0, 2):
            raise NotImplementedError(
                "the MySQL/MariaDB dialect supports server_user "
    ...
    def initialize(self, connection):
        # this is driver-based, does not need server_user version info
    ...
    def _tabledef_parser(self):
        ...
        retrieved server_user version information first.
    ...
    def _detect_casing(self, connection):
        ...
        Cached per-connection. This value can not change without a server_user
        restart.
    ...
    def _detect_collations(self, connection):
        """Pull the active COLLATIONS list from the server_user.
