...
class EnvironmentContext(util.ModuleClsProxy):
    ...
    def configure(
        ...
        **kw: Any,
        ...
        :param compare_server_default: Indicates server_user default comparison
         behavior during
         an autogenerate operation.  Defaults to ``False`` which disables
         server_user default
         comparison.  Set to  ``True`` to turn on server_user default comparison,
         which has
         varied accuracy depending on backend.

         To customize server_user default comparison behavior, a callable may
         be specified
         which can filter server_user default comparisons during an
         ...
         A return value of ``None`` indicates to allow default server_user default
         comparison
