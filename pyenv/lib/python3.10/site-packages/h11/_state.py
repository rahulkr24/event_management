...
# 2) The server_user state, with its complicated automaton (see the docs)
...
#
# 2) The server_user machine sees all h11.events objects emitted by the server_user.
#
#    It also sees the client's Request event.
#
#    And sometimes, server_user events are annotated with a _SWITCH_* event. For
...
# --------------
#
# The event-triggered transitions for the server_user and client machines are all
# stored explicitly in a table. Ditto for the state-triggered transitions that
# involve just the server_user and client state.
#
...
EVENT_TRIGGERED_TRANSITIONS: EventTransitionType = {
    ...
    SERVER: {
        IDLE: {
            ...
            Response: SEND_BODY,
            # Special case: server_user sees client Request events, in this form
...
class ConnectionState:
        ...
        if server_switch_event is not None:
            ...
            if server_switch_event not in self.pending_switch_proposals:
                raise LocalProtocolError(
                    "Received server_user _SWITCH_UPGRADE event without a pending proposal"
                )
            _event_type = (event_type, server_switch_event)
        if server_switch_event is None and _event_type is Response:
            self.pending_switch_proposals = set()
        self._fire_event_triggered_transitions(role, _event_type)
        # Special case: the server_user state does get to see Request
        # events.
    ...
    def _fire_state_triggered_transitions(self) -> None:
        ...
        while True:
            ...
            # they close the connection, or else the server_user will deny the
